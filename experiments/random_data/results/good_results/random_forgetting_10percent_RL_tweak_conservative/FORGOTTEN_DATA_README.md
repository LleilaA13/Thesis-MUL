# Forgotten Data Extraction - Random 10% Forgetting

## Overview
This directory contains the forgotten data from the **Random 10% Forgetting** experiment using the **RL (Re-Labeling) method with conservative tweaking**.

## Dataset Details
- **Total forgotten samples**: 10,000 images
...
- `forgotten_indices.pt`: PyTorch tensor containing the 10,000 indices of forgotten samples
- `forgotten_images/`: Directory with extracted image files
- `RLcheckpoint.pth.tar`: Unlearning checkpoint (96MB)
- `RLeval_result.pth.tar`: Evaluation results (963B)
- `with_0.6.pt`: Saliency mask at threshold 0.6

## Sample Index Statistics
- **Min index**: 6
- **Max index**: 99,980
- **First 10 indices**: [6, 9, 17, 23, 25, 26, 36, 48, 51, 52]

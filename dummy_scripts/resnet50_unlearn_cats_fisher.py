...
SAVE_DIR = os.path.join(current_dir, f"models/resnet50_{FORGET_TYPE}_forgetting/mask0_5_fisher")
...
# === Step 4: Run Fisher Information Unlearning ===
print("\n[*] Running Fisher Information unlearning using main_random.py...")
...
subprocess.run([
    ...
    "--unlearn", "fisher",  # Fisher Information Matrix method
    "--unlearn_epochs", "5",  # Fisher typically needs more epochs than GA
    "--unlearn_lr", "0.01",  # Fisher can handle higher LR than GA
    "--num_indexes_to_replace", str(num_to_forget),
    "--model_path", MODEL_PATH,
    "--save_dir", SAVE_DIR,
    "--mask_path", MASK_PATH,
...
print("\n[✓] Cat-class forgetting complete using Fisher Information + ResNet-50.")
print("[*] Cat-FISHER-METHOD parameters applied:")
print(f"    - Method: Fisher Information Matrix (theoretically grounded)")
print(f"    - Learning rate: 0.01 (Fisher can handle higher LR)")
print(f"    - Epochs: 5 (Fisher needs more iterations)")
print(f"    - Mask: 0.5 (blocks 50% of weights)")
print(f"    - COMPARISON: Fisher vs GA (21% forget, 53.73% retain)")
print(f"    - EXPECTATION: Better retain accuracy preservation")
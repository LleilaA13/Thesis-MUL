...
SAVE_DIR = os.path.join(current_dir, f"models/resnet50_{FORGET_TYPE}_forgetting/mask0_5_wfisher")
...
# === Step 4: Run Weighted Fisher Unlearning ===
print("\n[*] Running Weighted Fisher unlearning using main_random.py...")
...
subprocess.run([
    ...
    "--unlearn", "wfisher",  # Weighted Fisher Information method
    "--unlearn_epochs", "3",  # Weighted Fisher more efficient than regular Fisher
    "--unlearn_lr", "0.005",  # Conservative LR for Fisher
    "--num_indexes_to_replace", str(num_to_forget),
    "--model_path", MODEL_PATH,
    "--save_dir", SAVE_DIR,
    "--mask_path", MASK_PATH,
...
print("\n[✓] Cat-class forgetting complete using Weighted Fisher + ResNet-50.")
print("[*] Cat-WEIGHTED-FISHER parameters applied:")
print(f"    - Method: Weighted Fisher Information (advanced Fisher variant)")
print(f"    - Learning rate: 0.005 (conservative Fisher LR)")
print(f"    - Epochs: 3 (efficient weighted approach)")
print(f"    - Mask: 0.5 (blocks 50% of weights)")
print(f"    - COMPARISON: WFisher vs GA (21% forget, 53.73% retain)")
print(f"    - EXPECTATION: Best retain accuracy preservation with good forgetting")
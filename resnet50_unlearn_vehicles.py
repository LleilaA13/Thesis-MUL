...
if not os.path.exists(FORGET_MASK_PATH):
    ...
    # Create boolean mask using centralized config
    ...
    print(f"[✓] Created forget mask with {forget_mask.sum().item()} samples to forget")
else:
...
subprocess.run([
    ...
    "--unlearn_epochs", "5",  # Increased from 1
    "--unlearn_lr", "0.01",   # Keep at 0.01 - RL needs higher LR
    "--num_indexes_to_replace", str(num_to_forget),
    "--model_path", MODEL_PATH,
    "--save_dir", SAVE_DIR,
    "--mask_path", MASK_PATH,
